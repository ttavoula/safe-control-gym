    ...
    def _execute_evaluations(self, n_episodes=None, n_steps=None, log_freq=None, seeds=None):
        ...
        if n_episodes is not None:
            while trajs < n_episodes:
                # the uncertified controller proposes an action, 
                # sends the current state and proposed action to the safety filter, 
                # and gets back a certified action which it feeds into the system.
                action = self._select_action(obs=obs, info=info)
                # inner sim loop to accomodate different control frequencies
                for _ in range(sim_steps):
                    # state propagation
                    obs, _, done, info = self.env.step(action)
                    print(obs) # print propagated states
                    if done:
                        trajs += 1
